...
AIInterface::AIInterface(string execPath) : executablePath(execPath) {
  action = "pass";
}
AIInterface::~AIInterface() {}

int AIInterface::StartClientAi()
{
...
int AIInterface::connection_handler(int connection_fd)
{
  char rdb[1000];
...
template <class DataType> AIInterface& AIInterface::operator<< (const DataType& variable)
{
  stringstream s;
  s << variable << "\n";
...
template <class DataType> AIInterface& AIInterface::operator>> (DataType& variable)
{
  char rdb[1000];
  int n = read(connection_fd, rdb, sizeof(rdb));
...
PlayerMotionType AIInterface::getMovement()
{
  //    operator>>(action);
  ...
  if (action == "pass") {
    ...
    LOG_ERROR("Invalid motion: %s", action.c_str());
    action = "pass";
    return PASS;
  }
}
